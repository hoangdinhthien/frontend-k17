console.log( '06-PrototypeMethod-ObjectWithoutProto' );
...
//muốn truy cập vào [[Prototype]] dùng __proto__
...
Object.setPrototypeOf( rabbit, {} ); //thế kỉ 21
console.log( rabbit );
...
Vì thế các trình duyệt thêm một thuộc tính "không thuộc JS tiêu chuẩn" là _proto_ để cho phép
người dùng có thể get/set một prototype bất cứ lúc nào.
Sau đó, vào năm 2015, Object.setPrototypeOf và Object.getProtypeOf được thêm vào JS tiêu chuẩn,
để thực hiện chức năng tương tự như__proto__.
...
Về mặt kỹ thuật, chúng ta có thể get/set [[Prototype]] bất kỳ lúc nào. Nhưng thường thì chúng ta
chỉ set một lần khi object khởi tạo và không thay đổi nó nữa: rabbit kế thừa từ animal,
và nó sẽ không thay đổi.

Và các Javascript engine được tối ưu hóa cao cho việc này. Thay đổi một prototype "đang hoạt động"
với Object.setPrototypeOf hoặc obj.__proto__= là một phép tất rất chậm bởi vì nó phá vỡ sự tối ưu hóa
nội bộ cho các hoạt động truy cập đến thuộc tính object. Vì thế tránh sử dụng nó nếu bạn không biết
nó làm gì, hoặc tốc độ Javasript không phải là vấn đề bạn quan tâm.
*/


// very plain object | object siêu phẳng | base object
// String không thể trở thành 1 [[Prototype]]
obj = {};
let key = prompt( 'Nhập vào key', '__proto__' );
obj[key] = 'giá trị bất kỳ nào đó';
console.log( obj );
// __proto__ chỉ có thể là null hoặc object dạng bth, chứ ko thể là 1 string
...
Object.create(proto, [descriptors]) - Tạo một object rỗng với việc cung cấp proto như [[Prototype]] 
                                        và tùy chọn là các bộ mô tả thuộc tính (property descriptors)
Object.getPrototypeOf(obj) - return về [[Prototype]] của obj
Object.setPrototypeOf(obj, proto) - set [[Prototype]] của obj thành proto
getter/setter bằng _proto_ thì không an toàn nếu chúng ta muốn người dùng tự tạo key cho object. 
                            Bởi vì nếu người dùng nhập "_proto_" như là key, 
                            và nó sẽ gây ra lỗi, và đôi khi sẽ gây ra các kết quả không mong muốn.

Vì thế chúng ta có thể sử dụng Object.create(null) để tạo một "very plain" object mà không 
có _proto_, hoặc chuyển qua Map object.
...
Chúng ta cũng cần rõ ràng là _proto_ là getter/setter cho [[Prototype]] và 
nằm trong Object.prototype, như là các phương thức khác.

Chúng ta có thể tạo một object mà không có prototype bằng cách Object.create(null). 
Những object như vậy được sử dụng như là "pure dictionaries", chúng không có vấn đề gì với 
key "_proto_".
...
Tất cả các phương thức mà return về thuộc tính object (như Object.keys và một số khác) - chỉ return về thuộc tính của nó. 
Nếu chúng ta muốn các thuộc tính mà nó kế thừa, chúng ta có thể sử dụng for...in
*/
